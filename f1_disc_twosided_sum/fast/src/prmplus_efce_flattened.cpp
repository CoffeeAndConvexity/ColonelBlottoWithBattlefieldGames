...
PrmPlusEfceFlattened::PrmPlusEfceFlattened(unsigned int size)
    : RegretMinimizerEfceFlattenedBase(size)
{
}

PrmPlusEfceFlattened::PrmPlusEfceFlattened()
...
void PrmPlusEfceFlattened::recommend(double *store, bool unused) const
{
    ...
    for (const auto &act : operations_)
    {
        ...
        for (unsigned int idx = act.simplex_index;
             ...
             idx++)
        {
            assertm(regret_[idx] >= 0, "Regret should be non-negative");
            t += regret_[idx];
        }
             ...
             idx++)
        {
            ...
            store[idx] = std::max(0.0, store[idx]);
        }
        ...
        for (unsigned int idx = act.simplex_index;
             ...
             idx++)
        {
            assertm(store[idx] >= 0, "Regret should be non-negative");
            t += store[idx];
        }
        for (unsigned int idx = act.simplex_index;
             idx < act.simplex_index + act.simplex_size;
...
void PrmPlusEfceFlattened::observe_loss(double *loss, double *old_result)
{
         ...
         ++i)
    {
             ...
             idx++)
        {
            ...
            regret_[idx] = std::max(0.0, regret_[idx]);
        }
...
void PrmPlusEfceFlattened::allocate_mem()
{
