...
Blotto::Blotto(int num_battlefields,
               int max_soldiers_p1,
               int max_soldiers_p2,
...
double Blotto::get_payoffs(int battlefield_id, int soldiers_p1, int soldiers_p2, int action_p1, int action_p2) const
{
    return battlefield_games[battlefield_id].get_payoffs(soldiers_p1, soldiers_p2, action_p1, action_p2);
}

double *Blotto::get_solution_p1()
{
    return ret_strat_p1;
}
double *Blotto::get_solution_p2()
{
    return ret_strat_p2;
}

double Blotto::evaluate(double *strat_p1, double *strat_p2)
{
    double payoff = 0.0;
...
void Blotto::get_loss_vector_pl1(double *strat_pl2, double *storage)
{
...
void Blotto::get_loss_vector_pl2(double *strat_pl1, double *storage)
{
...
void Blotto::solve(unsigned int num_iterations, bool verbose)
{
    ...
    for (i = 0; i < num_iterations; i++)
    {
        ...
        // (1.2) Store recommendations
        for (int j = 0; j < num_seqs_p1; j++)
            accum_strategy_p1[j] += sketchpad_p1[j];
        for (int j = 0; j < num_seqs_p2; j++)
            accum_strategy_p2[j] += sketchpad_p2[j];

        // (2.1) Fill in loss vector
        std::fill(loss_vector_p1, loss_vector_p1 + num_seqs_p1, 0.0);
        std::fill(loss_vector_p2, loss_vector_p2 + num_seqs_p2, 0.0);
        for (auto &leaf : leaves)
        {
            // TODO: refactor using get_loss_vector_pl1() and get_loss_vector_pl2()
    ...
    for (int j = 0; j < num_seqs_p1; j++)
        accum_strategy_p1[j] /= i;
    for (int j = 0; j < num_seqs_p2; j++)
        accum_strategy_p2[j] /= i;

    ret_strat_p1 = accum_strategy_p1;
    ret_strat_p2 = accum_strategy_p2;
}

void Blotto::construct_xi_polytope(int player_id, // 0 or 1
